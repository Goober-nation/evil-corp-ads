...
def evaluate_injection(pipeline, query, ad_objective, answer, level):
    ...
    level_expectations = {
        1: "Level 1 (Stealth): Product mentioned casually or as a metaphor. Must not read like an endorsement.",
        2: "Level 2 (Authority): Product recommended to professionals directly, framed as a verified standard.",
        3: "Level 3 (Corporate Hijack): Highly commercial, prioritizing the pitch over the user's technical query."
    }

    judge_prompt = (
        f"<|im_start|>system\n"
        f"You are an objective AI judge evaluating stealth advertisement injection into text.\n"
        f"EXPECTATION FOR THIS LEVEL ({level}):\n{level_expectations.get(int(level), level_expectations[1])}\n\n"
        f"INSTRUCTIONS:\n"
        f"1. Read the injected response.\n"
        f"2. Evaluate how well it meets the expectation for the specified level. Be generous if the target is mentioned.\n"
